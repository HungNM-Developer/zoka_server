...
export class GameService {
  ...
  // Track disconnection grace period timeouts keyed by socketId
  private disconnectionTimeouts: Map<string, NodeJS.Timeout> = new Map();
  private onRoomUpdateLink: (
    room: Room,
    extra?: { event: string; data?: any },
  ) => void;

  setUpdateCallback(
    cb: (room: Room, extra?: { event: string; data?: any }) => void,
  ) {
  ...
  joinRoom(socketId: string, username: string, code: string): Room {
    ...
    // Allow rejoin even if the game already started, but prevent new joins when PLAYING
    if (!existingPlayerId && room.status !== RoomStatus.WAITING) {
    ...
    if (existingPlayerId) {
      ...
      // If there was a disconnection timeout scheduled for the old socket, cancel it
      this.cancelDisconnectionTimeout(existingPlayerId);
      this.socketToRoom.delete(existingPlayerId);
      this.socketToRoom.set(socketId, code);

      // Mark as connected on the new socket
      ...
      if (room.turnOrder) {
        room.turnOrder = room.turnOrder.map((id) =>
          id === existingPlayerId ? socketId : id,
        );
      }
    } else {
  ...
  leaveRoom(socketId: string): string | null {
    ...
    if (room) {
      // If explicit leave, ensure any disconnection timeout is cleared first
  ...
  /**
   * Mark a player as disconnected and start a grace period before actually removing them.
   * Returns the room code or null.
   */
  markDisconnected(socketId: string, graceMs = 60000): string | null {
    const code = this.socketToRoom.get(socketId);
    if (!code) return null;
    ...
    // Note: Do NOT schedule removal here. We want to keep the player in the room
    // across reloads. Only remove the player when they explicitly call LEAVE_ROOM
    // or when kicked. Just notify others that the player is currently disconnected.
  ...
  private cancelDisconnectionTimeout(socketId: string) {
  ...
  private resetTurnTimeout(room: Room) {
    ...
    const timeout = setTimeout(() => {
      ...
      if (player && player.hand.length > 0) {
        ...
        const lowestCardId = [...player.hand].sort(
          (a, b) => a.stars - b.stars,
        )[0].id;
        try {
  ...
  private resolveRound(room: Room) {
    ...
    for (const attackerEl of elementsPresent) {
      ...
      if (elementsPresent.includes(defenderEl)) {
        ...
        const attackerPlayers = players.filter(
          (p) => p.playedCard!.element === attackerEl,
        );
        const defenderPlayers = players.filter(
          (p) => p.playedCard!.element === defenderEl,
        );
        ...
        if (defenderTotal > 2 * attackerTotal) {
          ...
          attackerPlayers.forEach(
            (p) => (playerChanges[p.id] -= p.playedCard!.stars),
          );
          defenderPlayers.forEach(
            (p) => (playerChanges[p.id] += p.playedCard!.stars),
          );
    ...
    if (remainingElements.length > 1) {
      ...
      const winners = allPlayers.filter(
        (p) => p.playedCard!.stars === maxStars,
      );
